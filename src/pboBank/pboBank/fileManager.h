...
namespace pboBank {
	...
	class fileManager {
	public:
		fileManager(std::vector<boost::shared_ptr<file>> filesInit);
		~fileManager();
		...
		void copyFileToBank(boost::shared_ptr<file> pFile, std::string originPath);
		...
		const std::vector<boost::shared_ptr<file>>& getFiles() { return files; }
	private:
